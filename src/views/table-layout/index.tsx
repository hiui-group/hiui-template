            ...
            <div style={{display: "flex", flexDirection: 'column', height: '100%'}}>
              ...
              <div style={{marginTop: '8px', flex: '1'}}>
                {displayDepartments.length === 0 ?
                  ...
                  <Tree data={displayDepartments} selectedId={selectDepartmentCode}
                        onSelect={e => {
            ...
            <div style={{display: 'flex', flexDirection: 'column', height: '100%'}}>
              <div style={{display: 'flex', marginBottom: '16px', marginTop: '4px'}}>
                ...
                <Select placeholder={'职位'} style={{flex: 1, marginRight: '12px', maxWidth: '246px'}}
                        data={JobSelectData} value={selectJobCode} onChange={e => setSelectJobCode(e as string)}/>
                <Select placeholder={'级别'} style={{flex: 1, marginRight: '12px', maxWidth: '246px'}}
                        data={LevelSelectData} value={selectLevelCode} onChange={e => setSelectLevelCode(e as string)}/>
                <Button onClick={() => {
                  reset()
                  setFetchingPersonToggle(pre => pre + 1)
                }}>重置</Button>
                  ...
                  columns={[{
                    ...
                    render: (_t, record) => (
                      <div style={{display: 'flex', alignItems: 'center'}}>
                        <Avatar size={'sm'} src={record.avatar} shape={'square'}
                        ...
                        <div>
                          ...
                          <div style={{color: '#5F6A7A', fontSize: '12px', lineHeight: '20px'}}>{record.jobNumber}</div>
                        </div>
                      </div>
                    )
                  }, {
                    ...
                    render: (_t, record) => (
                      <div>
                        <div style={{color: '#1F2733', fontSize: '14px', lineHeight: '20px'}}>{record.miCode}</div>
                        <div style={{color: '#5F6A7A', fontSize: '12px', lineHeight: '20px'}}>{record.phone}</div>
                      </div>
                    )
                  }, {
                    title: '职级',
                    ...
                    render: (_t, record) => (
                      <div>
                        <div style={{color: '#1F2733', fontSize: '14px', lineHeight: '20px'}}>{record.job.title}</div>
                        <div style={{color: '#5F6A7A', fontSize: '12px', lineHeight: '20px'}}>{record.level.title}</div>
                      </div>
                    )
                  }, {
                    title: '邮箱',
