...
11. Build pages with package imports:
   - `@hiui-design/typical-page-shells`
   - `@hiui-design/typical-page-shells/host`
12. Keep route wiring, layout CSS, and real portals inside the host project.
13. If the target project uses Vite, alias `@hi-ui/schema-types` to `src/typical-page-reuse/shims/schema-types-empty.js`.
14. Keep the host header slot full width so `PageHeader extra` actions stay right-aligned after portal mounting.
15. If a page writes raw `SchemaForm` and contains `CheckSelect`, wrap it with `TypicalPageFieldMapProvider` from `@hiui-design/typical-page-shells`.
