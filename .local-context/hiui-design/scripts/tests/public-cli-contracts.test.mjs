...
test('public host-integration dependency manifest is shipped with bootstrap scripts', async () => {
  const manifest = await readJson('docs/onboarding/host-integration-dependencies.json')
  assert.equal(typeof manifest.dependencies, 'object')
  assert.equal(manifest.dependencies['react-router-dom'], '6.30.3')
})
