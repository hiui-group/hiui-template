...
20. `split shell inheritance strategy`（仅在 `layout archetype = context-main-split` 时输出）
21. `split shell carrier path`（仅在 `layout archetype = context-main-split` 时输出）
22. `split resize strategy`（仅在 `layout archetype = context-main-split` 时输出）
23. `resize handle selector`（仅在 `layout archetype = context-main-split` 时输出）
24. `split pane contract`（仅在 `layout archetype = context-main-split` 时输出）
25. `split risk check`（仅在 `layout archetype = context-main-split` 时输出）
26. `split-section closure strategy`（仅在 `layout strategy = primary-secondary` 且后方仍有同级通栏主内容时输出）
27. `chart baseline checklist`（仅在页面实际渲染图表时输出）
28. `table hard-gate verdicts`（仅在表格类页型或右侧表格工作区场景时输出）
