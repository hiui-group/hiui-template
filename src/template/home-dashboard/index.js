...
export default class HomeDashboard extends Component {
  ...
  async componentDidMount() {
    ...
    const {
      data: { data = [] }
    } = await Axios.get('https://yapi.baidu.com/mock/64112/hiui/dashboard/tabs')
  ...
  async updatePageInfos(id) {
    ...
    const fetchPageMatchIdInfo = async () => {
      const path = `https://yapi.baidu.com/mock/64112/hiui/dashboard/info?id=${id}`
      // 根据id获取页面信息
      const {
        data: {
          data: {
  ...
  async updateLogisticsInfo(id, start, end) {
    ...
    const path = `https://yapi.baidu.com/mock/64112/hiui/dashboard/logistics?id=${id}&start=${start}&end=${end}`
    // 根据id获取页面信息
    const {
      data: {
  ...
  async updateEnquiryOrderInfo(id, logisticsId, timeId) {
    ...
    const path = `https://yapi.baidu.com/mock/64112/hiui/dashboard/order?id=${id}&logisticsId=${logisticsId}&timeId=${timeId}`
    // 根据id获取页面信息
    const {
      data: {
        data: { enquiry = [], order = [] }
