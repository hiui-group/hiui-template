            ...
            {menuList.map((item) => {
              return (
                <Tabs.Pane key={item.id} tabTitle={item.title} tabId={item.id}>
                  <BasicUserSettings {...this.state} submitData={this.submitData} onChange={this.onValueChange} />
                </Tabs.Pane>
              );
            })}
          </Tabs>
          <Tabs onClick={this.handleClick} data={menuList}></Tabs>
        </Form>
      </div>
    );
  }
...
function BasicUserSettings({
  ...
  return (
    <div style={{ width: 592, marginTop: 24 }}>
      <FormItem label="头像">
        <img
          alt="头像"
          src={photo}
      ...
      <FormItem label="工卡号">
        <Input
          value={cardNum}
          placeholder={"请输入"}
          onChange={e => onChange("cardNum", e.target.value)}
        />
      </FormItem>
      <FormItem label="所在部门">
        <Cascader
