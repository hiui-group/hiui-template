...
export default class HomeDashboard extends Component {
  ...
  async componentDidMount() {
    ...
    window.onresize = Debounce(() => {
      Array.from(this.eChartRefMap.values()).forEach(chart => {
    ...
    const {
      data: { data = [] }
    } = await Axios.get('https://yapi.smart-xwork.cn/mock/64112/hiui/dashboard/tabs')
  ...
  async updatePageInfos(id) {
    ...
    const fetchPageMatchIdInfo = async () => {
      const path = `https://yapi.smart-xwork.cn/mock/64112/hiui/dashboard/info?id=${id}`
      // 根据id获取页面信息
      const {
        data: {
          data: {
  ...
  async updateLogisticsInfo(id, start, end) {
    ...
    const path = `https://yapi.smart-xwork.cn/mock/64112/hiui/dashboard/logistics?id=${id}&start=${start}&end=${end}`
    // 根据id获取页面信息
    const {
      data: {
        data: { headOffice = [], eightOffice = [] }
      }
    } = await Axios.get(path)
  ...
  async updateEnquiryOrderInfo(id, logisticsId, timeId) {
    ...
    const path = `https://yapi.smart-xwork.cn/mock/64112/hiui/dashboard/order?id=${id}&logisticsId=${logisticsId}&timeId=${timeId}`
    // 根据id获取页面信息
    const {
      data: {
        data: { enquiry = [], order = [] }
    ...
    return (
      <Col>
        {tabInfos.map(({ title, id }) => {
          ...
          return (
            <span onClick={() => changeActiveTabDel(id)} key={id} className={tagClassName}>
              {title}
            </span>
          )
        })}
  ...
  renderLogisticsChart() {
    const {
      ...
      logisticsStart,
      logisticsEnd,
      logisticsInfo: { headOffice, eightOffice }
    } = this.state
    // 物流表格表格配置
    const chartOption = {
      tooltip: {
  ...
  renderOrderInfoChart() {
    const {
      orderChartSelectLogisticCompanyId,
      orderChartSelectTimeRangeId,
      nowActiveTabId,
      enquiryAndOrderInfo: { enquiry, order },
      isLoadingData,
      isLoadingEnquiryOrder
    } = this.state
    const chartOption = {
      tooltip: {
        trigger: 'axis',
    ...
    const logisticsCompanyTitle = logisticsCompanyInfos.find(item => item.id === orderChartSelectLogisticCompanyId)
      .title
  ...
  renderExpressTypeChart() {
    ...
    const chartData = [...expressTypeInfos].map(item => ({ name: item.name + '快递', value: item.value }))
  ...
  renderActivityExpectChart() {
    ...
    // 生成Y周分割段
    const pieces = segmentation.map((item, index) => {
      ...
      series: [
        {
          ...
          axisLine: {
            lineStyle: {
              ...
              color: [
                [0, '#4387f4'],
                [safeEfficiencyRate / 100, '#4387f4'],
                [safeEfficiencyRate / 100, '#aaa'],
                [1, '#aaa']
              ]
            }
          },
  ...
  render() {
    ...
    return (
      <div className="page page--dashboard">
        ...
        <Row>{this.renderActivityExpectChart()}</Row>
        <Row gutter>
