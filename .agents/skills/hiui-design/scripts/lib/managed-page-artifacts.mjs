...
export async function syncManagedPageRegistry(targetRoot) {
  const contractsDir = getRulesOnlyPageContractsDir(targetRoot)
  await fsp.mkdir(path.dirname(getManagedPagesRegistryJsonPath(targetRoot)), { recursive: true })
  ...
  await fsp.writeFile(
    getManagedPagesRegistryJsonPath(targetRoot),
    `${JSON.stringify(registry, null, 2)}\n`,
    'utf8'
  )
  await fsp.writeFile(
    getManagedPagesRegistryMarkdownPath(targetRoot),
    renderManagedPagesRegistryMarkdown(entries, brokenEntries),
    'utf8'
  )
